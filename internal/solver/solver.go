...
func (s *S) Solve(v vector.V) vector.V {
	res, ok := optimize(v, s.cs, s.tolerance)
	if !ok {
		res = expand(s.cs)
	}
...
func optimize(v vector.V, cs []constraint.C, tolerance float64) (vector.V, bool) {
	r := region.New(nil, tolerance)

	res := v
	for _, c := range cs {
		if !c.In(res) {
			s, ok := r.Add(c, tolerance)
			if !ok {
