...
func TestGenerateResult(t *testing.T) {
	...
	for _, c := range testConfigs {
		t.Run(c.name, func(t *testing.T) {
			if got, ok := generateResult(c.v, c.cs, tolerance); ok != c.success || !vector.Within(c.want, got, tolerance) {
				t.Errorf("generateResult() = %v, %v, want = %v, %v", got, ok, c.want, c.success)
			}
		})
	}
}
...
func TestGenerateSegment(t *testing.T) {
	...
	for _, c := range testConfigs {
		t.Run(c.name, func(t *testing.T) {
			got, ok := generateSegment(c.c, c.cs, tolerance)
			if ok != c.success {
