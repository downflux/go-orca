...
// cl calculates the truncated left cone relative to the agent position.
//
// N.B.: The center of the characteristic circle of the cone is either the start
// or end of the truncated line segment -- which side of the line segment is
// determined by if the agent is oriented "above" or "below" the line.
//
// N.B.: The calculated truncated left and right cones may be the same if the
// the line segment in p-space lies oblique to the agent -- that is, if the
// agent is approaching the wall from the side. Note that we are using this
// p-space artifact to do v-space calculations: we do not care about the
// "further" side of the wall in real life when we are about to collide with the
// nearest side first.
func (c C) lc() *cone.C {
	t := c.segment.L().T(c.agent.P())
	d := c.segment.L().Distance(c.agent.P())
...
// line.
func (c C) l() vector.V { return c.lc().L() }
